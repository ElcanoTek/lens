...
main() {
  if [[ "${1:-}" == --help ]]; then
    echo 'Usage: sudo bash install.sh [--help] (installs main into /opt/lens-src)'
    exit 0
  fi
  ...
  local src="${LENS_SRC_DIR:-/opt/lens-src}"
  if [[ -e "$src" ]]; then
    echo "$src already exists. Use lens update, or run its scripts/bootstrap.sh to reconfigure." >&2
    exit 1
  fi
  dnf install -y git ca-certificates
  git clone --branch main --single-branch https://github.com/ElcanoTek/lens.git "$src"
