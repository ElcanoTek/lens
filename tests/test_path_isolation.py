...
"""Guards for the suite-wide managed-files isolation in the root ``conftest.py``.
...
The static scan below walks every top-level module of the checkout, so this file
does not have to live next to the conftest it guards. ``test_root_level_isolation.py``
at the repo root covers the one thing a test under ``tests/`` structurally
cannot: that the fixture reaches test modules outside ``tests/`` as well.
"""
...
def _module_level_path_assignments() -> list[tuple[str, str]]:
    ...
    for source_file in sorted(REPO_ROOT.glob("*.py")):
        # Test modules and the root conftest are the isolation machinery itself,
        # not product code it protects: the conftest necessarily names
        # `managed-files` paths in the registry it applies.
...
def test_every_module_path_global_is_isolated_or_recorded_read_only(path_isolation_registry):
    isolated = {
        (module, attribute) for module, attribute, _ in path_isolation_registry.module_globals
    }
    unaccounted = [
        entry
    ...
    assert not unaccounted, (
        ...
        "isolated by the root conftest.py's _MODULE_PATH_GLOBALS nor recorded "
        "as read-only in _READ_ONLY_PATH_GLOBALS: "
...
def test_every_config_path_attribute_is_isolated(path_isolation_registry):
    isolated = {attribute for attribute, _ in path_isolation_registry.config_attrs}
    candidates = {
        name
    ...
    assert candidates <= isolated, (
        "config attributes naming a filesystem location that the root "
        f"conftest.py does not isolate: {sorted(candidates - isolated)}"
    )
...
def test_fixture_redirects_config_paths(path_isolation_registry, tmp_path):
    for attribute, _ in path_isolation_registry.config_attrs:
...
def test_opt_out_marker_restores_the_real_paths(path_isolation_registry):
    ...
    assert path_isolation_registry.marker == "real_managed_files"
