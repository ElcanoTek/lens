...
The dashboard's Advanced settings populate two model dropdowns from OpenRouter's `/models` endpoint (`_get_model_catalog` in `web_service.py`, cached 1h): the **classification model** (must support tool calling; price-capped by `MODEL_PROMPT_PRICE_CAP`/`MODEL_COMPLETION_PRICE_CAP` — classification is high-volume and doesn't benefit from frontier-priced models) and the **research model** (must advertise `web_search_options`, i.e. built-in web search; capped by `RESEARCH_*_PRICE_CAP`). `~vendor/…-latest` aliases (OpenRouter's self-updating pins) sort first; `~openai/gpt-luna-latest` and `perplexity/sonar-pro` are the recommended defaults. A `config.json` still pinning the previous shipped classifier `~google/gemini-flash-latest` is upgraded on load. Selections reach the job subprocess via `--llm-model` / `--research-model`, choices persist per browser in localStorage, and non-default settings show on the run's row in the Runs table.
